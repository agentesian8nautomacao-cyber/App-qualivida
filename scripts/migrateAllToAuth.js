      ...
      if (createError) {
        ...
        try {
          ...
          if (listErr) {
            ...
            const found = (authList?.users || authList || []).find(u => (u.email || '').toLowerCase() === email);
            if (found) {
...
(async () => {
  try {
    // Process resident
    await migrateTable('resident');

    // Prefer staff, fallback to users
    const { data: staffExists } = await supabase.from('pg_catalog.pg_class').select('relname').eq('relname', 'staff').limit(1);
    if (staffExists && staffExists.length > 0) {
      await migrateTable('staff');
    } else {
      // try users
      const { data: usersExists } = await supabase.from('pg_catalog.pg_class').select('relname').eq('relname', 'users').limit(1);
      if (usersExists && usersExists.length > 0) {
        await migrateTable('users');
      } else {
        console.log('Nenhuma tabela staff ou users encontrada.');
      }
    }
